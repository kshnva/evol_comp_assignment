...
# ------------------------ Observation ------------------------ #
def build_observation(model, data):
    """Return only joint angles (exclude root pos + orientation)."""
    qpos = np.array(data.qpos).ravel()
    joint_angles = qpos[7:] if len(qpos) > 7 else qpos
...
def main():
    ...
    # Build controller
    obs_vec = build_observation(model, data)
    obs_dim = len(obs_vec) #Nr of joints
    nu = model.nu #Nr of actuators
